...
namespace NongDanService.Controllers
{
    ...
    public class NongDanController : ControllerBase
    {
        ...
        /// <summary>
        /// Lấy nông dân theo tài khoản
        /// </summary>
        /// <param name="maTaiKhoan">Mã tài khoản</param>
        /// <returns>Thông tin nông dân</returns>
        [HttpGet("get-by-account/{maTaiKhoan}")]
        public IActionResult GetByAccount(int maTaiKhoan)
        {
            try
            {
                if (maTaiKhoan <= 0)
                ...
                var data = _nongDanService.GetByAccount(maTaiKhoan);
                if (data == null)
                {
                    return NotFound(new
                    {
                        success = false,
                        message = "Không tìm thấy nông dân với tài khoản này"
                    });
                }
